export const locale: { [key: string]: { [key: string]: string } } = {
  fr: {
    ...
    app_description_must_be_string:
      "La description de l'application doit être une chaîne de caractères.",
    app_component_must_be_string:
      "Le composant de l'application doit être une chaîne de caractères.",
    app_once_must_be_boolean:
      "La propriété 'once' de l'application doit être un booléen.",
    app_close_must_be_boolean:
      "La propriété 'close' de l'application doit être un booléen.",
    app_max_must_be_int:
      "La valeur maximale de l'application doit être un entier.",
    app_message_must_be_string:
      "Le message de l'application doit être une chaîne de caractères.",
    app_metadata_must_be_object:
      "Les métadonnées de l'application doivent être un objet.",
    app_steps_must_be_object:
      "Les étapes de l'application doivent être un objet.",
    app_name_length_must_be_greater_than:
      "La longueur du nom de l'application doit être supérieure à {value}.",
    app_name_length_must_be_less_than:
      "La longueur du nom de l'application doit être inférieure à {value}.",
    app_max_must_be_less_than:
      "La valeur maximale de l'application doit être inférieure à {value}.",
  ...
  en: {
    app_name_must_be_string: "The application name must be a string.",
    app_description_must_be_string:
      "The application description must be a string.",
    app_component_must_be_string: "The application component must be a string.",
    app_once_must_be_boolean:
      "The 'once' property of the application must be a boolean.",
    app_close_must_be_boolean:
      "The 'close' property of the application must be a boolean.",
    app_max_must_be_int:
      "The maximum value of the application must be an integer.",
    app_message_must_be_string: "The application message must be a string.",
    app_metadata_must_be_object: "The application metadata must be an object.",
    app_steps_must_be_object: "The application steps must be an object.",
    app_name_length_must_be_greater_than:
      "The application name length must be greater than {value}.",
    app_name_length_must_be_less_than:
      "The application name length must be less than {value}.",
    app_max_must_be_less_than:
      "The maximum value of the application must be less than {value}.",
...
export function getLibellys(key: string) {
  let value = "";
  if (key.includes(":")) {
    key = key.split(":")[1];
    value = key.split(":")[1];
  ...
  const libellys: { [key: string]: string } = {};
  for (const lang in locale) {
    if (Object.prototype.hasOwnProperty.call(locale, lang)) {
      libellys[lang] = locale[lang][key].replace("{value}", value);
    }
  }
